...
def create_tables():
    ...
    try:
        ...
        trigger_text = triggers_path.read_text(encoding="utf-8")

        trigger_blocks = [
            b.strip()
            for b in trigger_text.split("-- TRIGGER_END")
            if b.strip()
        ]
        ...
        procedure_text = procedures_path.read_text(encoding="utf-8")

        proc_blocks = [b.strip() for b in procedure_text.split("-- PROC_END") if b.strip()]
